...
const OrderDetailsForm = ({
  ...
  }, [checkedextras, product.price, product.extraGroups, quantity, form]);
  ...
  function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    selectedExtras.forEach((extraId) => {
      ...
      if (extraGroup) {
        const extra = extraGroup.extras.find((extra) => extra.id === extraId);
        if (extra) {
          extras.push(extra);
        }
      }
    ...
    if (existingProductIndex !== -1) {
      ...
      updatedOrderDetails[existingProductIndex] = {
        ...updatedOrderDetails[existingProductIndex],
        quantity:
          updatedOrderDetails[existingProductIndex].quantity + data.quantity,
            ...
            product.extraGroups.map((extraGroup) => (
              <div key={extraGroup.id} className="space-y-4">
                <div className="flex extras-center gap-2">
                  <p className="text-lg font-semibold">{extraGroup.name} - </p>
                  <span className="text-sm text-gray-400">
                    Choose from {extraGroup.min} to {extraGroup.max}
                  </span>
                </div>
            ...
            render={({ field }) => (
              <div className="flex extras-center justify-center">
                <Button
                  ...
                  variant={"outline"}
                  onClick={() => {
                    const currentValue = parseInt(String(field.value));
